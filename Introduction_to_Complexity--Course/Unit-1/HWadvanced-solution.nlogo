...
to set-up-pheromone
  ask patches [
    set pheromone 0
    set nest? (distancexy 0 0)< 2       ; 判断是否是巢穴 （距离远点不超过2 即9个像素点）
    if nest? [set pcolor orange] ]     ; 如果是 巢穴 。颜色更换
end
...
to return-to-nest
  ...
  if pcolor = orange
  [set return-to-nest? false]    ;如果已经在巢穴里面了 ，返回false
end
...
to look-for-food
  ...
  let pheromone-left?  scent-at-angle -45     ; 信息素的位置所在


  ifelse (pheromone-right? > pheromone-ahead?  or pheromone-left? > pheromone-ahead?)
  [ifelse pheromone-right? > pheromone-left?
    [rt 45 ]
    [lt 45 ]]    ; The turtle turns left by number degrees.

  [if pheromone-ahead? = 0     ; 前方没有像素点
    ...
    [
      ...
      set return-to-nest? true     ; 吃完食物返回巢穴
...
to-report scent-at-angle [angle]  ; 相当于 函数 func 。[angle] --相当于input
  let p patch-right-and-ahead angle 1   ; p 是目前目标点 右前方一定角度的一个点
  if p = nobody [report 0] ;立即退出当前的报告程序，并根据该程序报告值
